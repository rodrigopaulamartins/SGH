...
begin
   ...
   try
     ...
     DM.Conexao.Connected := True;
       ...
       Application.MessageBox('Problemas de conexão com o banco de dados, contate o Administrador.','Conexão com banco de dados', MB_ICONERROR + MB_OK);
       abort;
   end;
end;
...
begin
   //validação do banco de dados login e senha
   LerIni;
   //se validou a conexão verifico os dados
   if Edt_Usuario.Text = '' then
   begin
      Application.MessageBox('Informe o nome do usuário.','Obrigatório', MB_ICONINFORMATION+MB_OK);
   ...
   begin
      Application.MessageBox('Informe a senha de acesso.','Obrigatório', MB_ICONINFORMATION+MB_OK);
      Edt_Senha.SetFocus;
      abort;
   end;
   ...
   QLogin.Open;
   //se não encontrar faz o alerta
   if QLogin.IsEmpty then
   begin
      Application.MessageBox('Usuário ou senha inválidos.','Login de acesso', MB_ICONWARNING+MB_OK);
      abort;
   end;
