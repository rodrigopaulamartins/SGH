object FInclusaoAlteracaoEspecialidade: TFInclusaoAlteracaoEspecialidade
  Left = 0
  Top = 0
  BorderIcons = []
  ClientHeight = 352
  ClientWidth = 812
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 46
    Width = 812
    Height = 243
    Align = alClient
    BevelOuter = bvNone
    Color = clWindow
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Roboto'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object Label2: TLabel
      Left = 72
      Top = 14
      Width = 39
      Height = 15
      Caption = 'C'#243'digo'
      FocusControl = DBEdit1
    end
    object Label3: TLabel
      Left = 169
      Top = 14
      Width = 59
      Height = 15
      Caption = 'Descri'#231#227'o'
      FocusControl = DBEdit2
    end
    object Label4: TLabel
      Left = 72
      Top = 71
      Width = 98
      Height = 15
      Caption = 'C'#243'digo CBO-SUS'
      FocusControl = DBEdit1
    end
    object Label5: TLabel
      Left = 72
      Top = 128
      Width = 98
      Height = 15
      Caption = 'C'#243'digo CBO-SUS'
      FocusControl = DBEdit1
    end
    object Edit1: TEdit
      Left = 16
      Top = 274
      Width = 121
      Height = 23
      TabOrder = 0
      Visible = False
    end
    object DBEdit1: TDBEdit
      Left = 72
      Top = 32
      Width = 81
      Height = 23
      TabStop = False
      DataField = 'codigo'
      DataSource = DCadEspecialidade
      ReadOnly = True
      TabOrder = 1
    end
    object DBEdit2: TDBEdit
      Left = 169
      Top = 32
      Width = 568
      Height = 23
      DataField = 'descricao'
      DataSource = DCadEspecialidade
      TabOrder = 2
      OnEnter = DBEdit2Enter
      OnExit = DBEdit2Exit
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 72
      Top = 89
      Width = 665
      Height = 23
      DataField = 'cod_cbosus'
      DataSource = DCadEspecialidade
      KeyField = 'cbo_sus'
      ListField = 'descricao'
      ListSource = DPesqCBOSUS
      TabOrder = 3
      OnEnter = DBLookupComboBox1Enter
      OnExit = DBLookupComboBox1Exit
    end
    object DBLookupComboBox2: TDBLookupComboBox
      Left = 72
      Top = 146
      Width = 665
      Height = 23
      DataField = 'cod_cbo'
      DataSource = DCadEspecialidade
      KeyField = 'tab_tis303'
      ListField = 'descricao'
      ListSource = DPesqCBOTIS
      TabOrder = 4
      OnEnter = DBLookupComboBox2Enter
      OnExit = DBLookupComboBox2Exit
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 289
    Width = 812
    Height = 63
    Align = alBottom
    BevelOuter = bvNone
    Color = clWindow
    ParentBackground = False
    TabOrder = 1
    object pnl_btnconfirmar: TPanel
      Left = 43
      Top = 14
      Width = 129
      Height = 46
      BevelInner = bvRaised
      BevelOuter = bvNone
      BevelWidth = 2
      Caption = 'Confirmar'
      Color = 16744448
      ParentBackground = False
      TabOrder = 0
      OnClick = pnl_btnconfirmarClick
    end
    object pnl_btncancelar: TPanel
      Left = 178
      Top = 14
      Width = 129
      Height = 46
      BevelInner = bvRaised
      BevelOuter = bvNone
      Caption = 'Cancelar'
      Color = 16119285
      ParentBackground = False
      TabOrder = 1
      OnClick = pnl_btncancelarClick
    end
  end
  object pnl_topo: TPanel
    Left = 0
    Top = 0
    Width = 812
    Height = 46
    Align = alTop
    BevelOuter = bvNone
    Color = clWindow
    ParentBackground = False
    TabOrder = 2
    object Label1: TLabel
      Left = 16
      Top = 12
      Width = 118
      Height = 19
      Caption = 'ESPECIALIDADE'
      Font.Charset = ANSI_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Roboto'
      Font.Style = [fsBold, fsUnderline]
      ParentFont = False
    end
    object btn_fechar: TSpeedButton
      Left = 784
      Top = 0
      Width = 28
      Height = 46
      Align = alRight
      ImageIndex = 1
      Images = ImageList1
      Flat = True
      OnClick = btn_fecharClick
      ExplicitLeft = 744
      ExplicitHeight = 33
    end
    object Panel4: TPanel
      Left = 739
      Top = 0
      Width = 45
      Height = 46
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 0
    end
  end
  object ImageList1: TImageList
    BlendColor = clHighlightText
    BkColor = clWhite
    Left = 576
    Top = 288
    Bitmap = {
      
      000000000000}
  end
  object QCadEspecialidade: TZQuery
    Connection = DM.Conexao
    SQL.Strings = (
      'SELECT * FROM ARQ_ESPECIALIDADE')
    Params = <>
    Left = 352
    Top = 286
    object QCadEspecialidadecodigo: TIntegerField
      FieldName = 'codigo'
      Required = True
    end
    object QCadEspecialidadedescricao: TWideStringField
      FieldName = 'descricao'
      Size = 50
    end
    object QCadEspecialidadecod_cbo: TWideStringField
      FieldName = 'cod_cbo'
      Size = 6
    end
    object QCadEspecialidadecod_cbosus: TWideStringField
      FieldName = 'cod_cbosus'
      Size = 6
    end
    object QCadEspecialidadebtn_alterar: TWideStringField
      FieldName = 'btn_alterar'
      Size = 1
    end
    object QCadEspecialidadebtn_excluir: TWideStringField
      FieldName = 'btn_excluir'
      Size = 1
    end
  end
  object DCadEspecialidade: TDataSource
    DataSet = QCadEspecialidade
    Left = 392
    Top = 286
  end
  object DPesqCBOSUS: TDataSource
    DataSet = QPesqCBOSUS
    Left = 464
    Top = 289
  end
  object DPesqCBOTIS: TDataSource
    DataSet = QPesqCBOTIS
    Left = 536
    Top = 286
  end
  object QPesqCBOSUS: TZQuery
    Connection = DM.Conexao
    SQL.Strings = (
      'SELECT * FROM ARQ_CBOSUS'
      'ORDER BY DESCRICAO')
    Params = <>
    Left = 432
    Top = 289
    object QPesqCBOSUScodigo: TIntegerField
      FieldName = 'codigo'
      Required = True
    end
    object QPesqCBOSUScbo_sus: TWideStringField
      FieldName = 'cbo_sus'
      Size = 6
    end
    object QPesqCBOSUSdescricao: TWideStringField
      FieldName = 'descricao'
      Size = 100
    end
  end
  object QPesqCBOTIS: TZQuery
    Connection = DM.Conexao
    SQL.Strings = (
      'SELECT * FROM ARQ_CBOTIS'
      'ORDER BY DESCRICAO')
    Params = <>
    Left = 504
    Top = 289
    object QPesqCBOTIScodigo: TIntegerField
      FieldName = 'codigo'
      Required = True
    end
    object QPesqCBOTISdescricao: TWideStringField
      FieldName = 'descricao'
      Size = 100
    end
    object QPesqCBOTIStab_tis303: TWideStringField
      FieldName = 'tab_tis303'
      Size = 6
    end
  end
end
