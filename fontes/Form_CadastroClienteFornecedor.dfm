object FCadastroClienteFornecedor: TFCadastroClienteFornecedor
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsNone
  Caption = 'Clientes/Fornecedores'
  ClientHeight = 495
  ClientWidth = 1120
  Color = clWindow
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1120
    Height = 47
    Align = alTop
    BevelOuter = bvNone
    Color = clWindow
    ParentBackground = False
    TabOrder = 0
    object Label1: TLabel
      Left = 16
      Top = 6
      Width = 136
      Height = 19
      Caption = 'Cliente/Fornecedor'
      Font.Charset = ANSI_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Roboto'
      Font.Style = [fsBold, fsUnderline]
      ParentFont = False
    end
    object SpeedButton1: TSpeedButton
      Left = 1087
      Top = 0
      Width = 33
      Height = 47
      Align = alRight
      ImageIndex = 2
      Images = ImageList1
      Flat = True
      OnClick = SpeedButton1Click
      ExplicitLeft = 608
      ExplicitHeight = 33
    end
    object SpeedButton2: TSpeedButton
      Left = 879
      Top = 2
      Width = 30
      Height = 30
      ImageIndex = 1
      Images = ImageList1
      Flat = True
      OnClick = SpeedButton2Click
    end
    object Edit1: TEdit
      Left = 356
      Top = 3
      Width = 517
      Height = 27
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Roboto'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object pnl_btnconfirmar: TPanel
      Left = 958
      Top = 0
      Width = 129
      Height = 47
      Align = alRight
      BevelInner = bvRaised
      BevelOuter = bvNone
      BevelWidth = 2
      Caption = 'Novo  +'
      Color = 16744448
      ParentBackground = False
      TabOrder = 1
      OnClick = pnl_btnconfirmarClick
    end
    object ComboBox1: TComboBox
      Left = 170
      Top = 3
      Width = 169
      Height = 27
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Roboto'
      Font.Style = []
      ItemIndex = 0
      ParentFont = False
      TabOrder = 2
      Text = 'Raz'#227'o Social'
      Items.Strings = (
        'Raz'#227'o Social'
        'Nome Fantasia'
        'CNPJ/CPF')
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 47
    Width = 1120
    Height = 448
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 1
    object DBGrid1: TDBGrid
      AlignWithMargins = True
      Left = 3
      Top = 5
      Width = 1114
      Height = 440
      Margins.Top = 5
      Align = alClient
      BorderStyle = bsNone
      DataSource = DCadCliente
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Roboto'
      Font.Style = []
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Roboto'
      TitleFont.Style = []
      OnCellClick = DBGrid1CellClick
      OnDrawColumnCell = DBGrid1DrawColumnCell
      Columns = <
        item
          Expanded = False
          FieldName = 'codigo'
          Title.Caption = 'C'#243'digo'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'cpf_cnpj'
          Title.Caption = 'CPF/CNPJ'
          Width = 107
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'razao_social'
          Title.Caption = 'Raz'#227'o Social'
          Width = 251
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'nome_fantasia'
          Title.Caption = 'Nome Fantasia'
          Width = 198
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'contato'
          Title.Caption = 'Contato'
          Width = 132
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'email'
          Title.Caption = 'E-mail'
          Width = 150
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'btn_alterar'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindow
          Title.Font.Height = -11
          Title.Font.Name = 'Roboto'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'btn_excluir'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindow
          Title.Font.Height = -11
          Title.Font.Name = 'Roboto'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'endereco'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'numero'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'idtplogradouro'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'complemento'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'cep'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'bairro'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'idcidade'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'uf'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'idpais'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'ddd'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'telefone1'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'ddd1'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'telefone2'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'site'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'idRamoatividade'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'dt_cadastro'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'inativo'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'cliente'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'fornecedor'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'pessoa_fisica'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'prestador_servico'
          Visible = False
        end>
    end
  end
  object ImageList1: TImageList
    BlendColor = clHighlightText
    BkColor = clWhite
    Left = 329
    Top = 240
    Bitmap = {
      
      000000000000}
  end
  object ImageList2: TImageList
    Left = 360
    Top = 241
    Bitmap = {
      
      000000000000}
  end
  object QPesqCadCliente: TZQuery
    Connection = DM.Conexao
    SQL.Strings = (
      'SELECT * FROM CAD_CLIENTES'
      'ORDER BY RAZAO_SOCIAL')
    Params = <>
    Left = 408
    Top = 239
    object QPesqCadClientecodigo: TIntegerField
      FieldName = 'codigo'
      Required = True
    end
    object QPesqCadClientecpf_cnpj: TWideStringField
      FieldName = 'cpf_cnpj'
    end
    object QPesqCadClienterazao_social: TWideStringField
      FieldName = 'razao_social'
      Size = 100
    end
    object QPesqCadClientenome_fantasia: TWideStringField
      FieldName = 'nome_fantasia'
      Size = 100
    end
    object QPesqCadClienteidtplogradouro: TIntegerField
      FieldName = 'idtplogradouro'
    end
    object QPesqCadClienteendereco: TWideStringField
      FieldName = 'endereco'
      Size = 100
    end
    object QPesqCadClientenumero: TWideStringField
      FieldName = 'numero'
      Size = 10
    end
    object QPesqCadClientecomplemento: TWideStringField
      FieldName = 'complemento'
      Size = 30
    end
    object QPesqCadClientecep: TWideStringField
      FieldName = 'cep'
      Size = 9
    end
    object QPesqCadClientebairro: TWideStringField
      FieldName = 'bairro'
      Size = 30
    end
    object QPesqCadClienteidcidade: TIntegerField
      FieldName = 'idcidade'
    end
    object QPesqCadClienteuf: TWideStringField
      FieldName = 'uf'
      Size = 2
    end
    object QPesqCadClienteidpais: TIntegerField
      FieldName = 'idpais'
    end
    object QPesqCadClienteddd: TWideStringField
      FieldName = 'ddd'
      Size = 2
    end
    object QPesqCadClientetelefone1: TWideStringField
      FieldName = 'telefone1'
    end
    object QPesqCadClienteddd1: TWideStringField
      FieldName = 'ddd1'
      Size = 2
    end
    object QPesqCadClientetelefone2: TWideStringField
      FieldName = 'telefone2'
    end
    object QPesqCadClienteemail: TWideStringField
      FieldName = 'email'
      Size = 100
    end
    object QPesqCadClientesite: TWideStringField
      FieldName = 'site'
      Size = 50
    end
    object QPesqCadClientecontato: TWideStringField
      FieldName = 'contato'
      Size = 50
    end
    object QPesqCadClienteidRamoatividade: TIntegerField
      FieldName = 'idRamoatividade'
    end
    object QPesqCadClientedt_cadastro: TDateField
      FieldName = 'dt_cadastro'
    end
    object QPesqCadClienteinativo: TWideStringField
      FieldName = 'inativo'
      Size = 1
    end
    object QPesqCadClientecliente: TWideStringField
      FieldName = 'cliente'
      Size = 1
    end
    object QPesqCadClientefornecedor: TWideStringField
      FieldName = 'fornecedor'
      Size = 1
    end
    object QPesqCadClientepessoa_fisica: TWideStringField
      FieldName = 'pessoa_fisica'
      Size = 1
    end
    object QPesqCadClienteprestador_servico: TWideStringField
      FieldName = 'prestador_servico'
      Size = 1
    end
    object QPesqCadClientebtn_alterar: TWideStringField
      FieldName = 'btn_alterar'
      Size = 1
    end
    object QPesqCadClientebtn_excluir: TWideStringField
      FieldName = 'btn_excluir'
      Size = 1
    end
  end
  object DCadCliente: TDataSource
    DataSet = QPesqCadCliente
    Left = 440
    Top = 239
  end
end
