object FPesquisaCadastroPestrador: TFPesquisaCadastroPestrador
  Left = 0
  Top = 0
  BorderIcons = []
  ClientHeight = 437
  ClientWidth = 883
  Color = clWindow
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object pnl_topo: TPanel
    Left = 0
    Top = 0
    Width = 883
    Height = 46
    Align = alTop
    BevelOuter = bvNone
    Color = clWindow
    ParentBackground = False
    TabOrder = 0
    ExplicitTop = -3
    object Label1: TLabel
      Left = 16
      Top = 12
      Width = 273
      Height = 19
      Caption = 'PESQUISA PRESTADOR DE SERVI'#199'OS'
      Font.Charset = ANSI_CHARSET
      Font.Color = clTeal
      Font.Height = -16
      Font.Name = 'Roboto'
      Font.Style = [fsBold, fsUnderline]
      ParentFont = False
    end
    object btn_fechar: TSpeedButton
      Left = 855
      Top = 0
      Width = 28
      Height = 46
      Align = alRight
      ImageIndex = 1
      Images = ImageList1
      Flat = True
      OnClick = btn_fecharClick
      ExplicitLeft = 744
      ExplicitHeight = 33
    end
    object Bevel1: TBevel
      Left = 852
      Top = 0
      Width = 3
      Height = 46
      Align = alRight
      Style = bsRaised
      ExplicitLeft = 411
      ExplicitHeight = 33
    end
    object SpeedButton2: TSpeedButton
      Left = 780
      Top = 6
      Width = 30
      Height = 30
      ImageIndex = 2
      Images = ImageList1
      Flat = True
      OnClick = SpeedButton2Click
    end
    object Edit1: TEdit
      Left = 304
      Top = 9
      Width = 461
      Height = 27
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Roboto'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 376
    Width = 883
    Height = 61
    Align = alBottom
    BevelOuter = bvNone
    Color = clWindow
    ParentBackground = False
    TabOrder = 1
    object pnl_btnconfirmar: TPanel
      Left = 16
      Top = 8
      Width = 129
      Height = 46
      BevelInner = bvRaised
      BevelOuter = bvNone
      BevelWidth = 2
      Caption = 'Selecionar'
      Color = 16744448
      ParentBackground = False
      TabOrder = 0
      OnClick = pnl_btnconfirmarClick
    end
    object pnl_btncancelar: TPanel
      Left = 148
      Top = 8
      Width = 129
      Height = 46
      BevelInner = bvRaised
      BevelOuter = bvNone
      Caption = 'Cancelar'
      Color = 16119285
      ParentBackground = False
      TabOrder = 1
      OnClick = pnl_btncancelarClick
    end
  end
  object DBGrid1: TDBGrid
    AlignWithMargins = True
    Left = 3
    Top = 51
    Width = 877
    Height = 322
    Margins.Top = 5
    Align = alClient
    BorderStyle = bsNone
    DataSource = DPesquisaClientes
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Roboto'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ParentFont = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Roboto'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'codigo'
        Title.Caption = 'C'#243'digo'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'razao_social'
        Title.Caption = 'Nome Completo'
        Width = 489
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'cpf_cnpj'
        Title.Caption = 'CPF/CNPJ'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'nome_fantasia'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'idtplogradouro'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'endereco'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'numero'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'complemento'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'cep'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'bairro'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'idcidade'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'uf'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'idpais'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'ddd'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'telefone1'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'ddd1'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'telefone2'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'email'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'site'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'contato'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'idRamoatividade'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'dt_cadastro'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'inativo'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'cliente'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'fornecedor'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'pessoa_fisica'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'prestador_servico'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'btn_alterar'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'btn_excluir'
        Visible = False
      end>
  end
  object ImageList1: TImageList
    BlendColor = clHighlightText
    BkColor = clWhite
    Left = 721
    Top = 48
    Bitmap = {
      
      000000000000}
  end
  object DPesquisaClientes: TDataSource
    DataSet = QPesquisaClientes
    Left = 472
    Top = 224
  end
  object QPesquisaClientes: TZQuery
    Connection = DM.Conexao
    SQL.Strings = (
      'SELECT * FROM CAD_CLIENTES'
      'WHERE IDRAMOATIVIDADE = '#39'7'#39
      'AND INATIVO = '#39'N'#39
      'ORDER BY RAZAO_SOCIAL')
    Params = <>
    Left = 440
    Top = 224
    object QPesquisaClientescodigo: TIntegerField
      FieldName = 'codigo'
      Required = True
    end
    object QPesquisaClientescpf_cnpj: TWideStringField
      FieldName = 'cpf_cnpj'
    end
    object QPesquisaClientesrazao_social: TWideStringField
      FieldName = 'razao_social'
      Size = 100
    end
    object QPesquisaClientesnome_fantasia: TWideStringField
      FieldName = 'nome_fantasia'
      Size = 100
    end
    object QPesquisaClientesidtplogradouro: TIntegerField
      FieldName = 'idtplogradouro'
    end
    object QPesquisaClientesendereco: TWideStringField
      FieldName = 'endereco'
      Size = 100
    end
    object QPesquisaClientesnumero: TWideStringField
      FieldName = 'numero'
      Size = 10
    end
    object QPesquisaClientescomplemento: TWideStringField
      FieldName = 'complemento'
      Size = 30
    end
    object QPesquisaClientescep: TWideStringField
      FieldName = 'cep'
      Size = 9
    end
    object QPesquisaClientesbairro: TWideStringField
      FieldName = 'bairro'
      Size = 30
    end
    object QPesquisaClientesidcidade: TIntegerField
      FieldName = 'idcidade'
    end
    object QPesquisaClientesuf: TWideStringField
      FieldName = 'uf'
      Size = 2
    end
    object QPesquisaClientesidpais: TIntegerField
      FieldName = 'idpais'
    end
    object QPesquisaClientesddd: TWideStringField
      FieldName = 'ddd'
      Size = 2
    end
    object QPesquisaClientestelefone1: TWideStringField
      FieldName = 'telefone1'
    end
    object QPesquisaClientesddd1: TWideStringField
      FieldName = 'ddd1'
      Size = 2
    end
    object QPesquisaClientestelefone2: TWideStringField
      FieldName = 'telefone2'
    end
    object QPesquisaClientesemail: TWideStringField
      FieldName = 'email'
      Size = 100
    end
    object QPesquisaClientessite: TWideStringField
      FieldName = 'site'
      Size = 50
    end
    object QPesquisaClientescontato: TWideStringField
      FieldName = 'contato'
      Size = 50
    end
    object QPesquisaClientesidRamoatividade: TIntegerField
      FieldName = 'idRamoatividade'
    end
    object QPesquisaClientesdt_cadastro: TDateField
      FieldName = 'dt_cadastro'
    end
    object QPesquisaClientesinativo: TWideStringField
      FieldName = 'inativo'
      Size = 1
    end
    object QPesquisaClientescliente: TWideStringField
      FieldName = 'cliente'
      Size = 1
    end
    object QPesquisaClientesfornecedor: TWideStringField
      FieldName = 'fornecedor'
      Size = 1
    end
    object QPesquisaClientespessoa_fisica: TWideStringField
      FieldName = 'pessoa_fisica'
      Size = 1
    end
    object QPesquisaClientesprestador_servico: TWideStringField
      FieldName = 'prestador_servico'
      Size = 1
    end
    object QPesquisaClientesbtn_alterar: TWideStringField
      FieldName = 'btn_alterar'
      Size = 1
    end
    object QPesquisaClientesbtn_excluir: TWideStringField
      FieldName = 'btn_excluir'
      Size = 1
    end
  end
end
